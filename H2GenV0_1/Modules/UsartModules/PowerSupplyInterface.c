...
void ProcessingMsgPowerSupply(volatile uint8_t * msg, volatile uint8_t * size , struct DataStruct * values);

void SendBytes(uint8_t * bytes, uint8_t size)
{
	SendBytesUART(bytes, size, PowerSupplyUart);
};
...
void StartPowerSupply()
{
	...
	val.AmperageSetpoint = 50;
	val.VoltageSetpoint = MAX_VOLTAGE;
	while (!SendCmdToPowerSupply(TIMEOUT_CONF_CMD, &val)) {};
	while (!SendCmdToPowerSupply(BIT_POWER_ON_CMD, &val)) {};
	while (!SendCmdToPowerSupply(SET_VOLT_AMP_CMD, &val)) {};
	while (!SendCmdToPowerSupply(BIT_WORK_ON_CMD, &val)) {};
...
{
	InitUartCallback(PowerSupplyUart, onGetBytesPowerSupply, onSendBytesPowerSupply);
};
...
{
	ProcessingMsgPowerSupply(PowerSupplyBuff, &PowerSupplyBuffLen, values);
};
